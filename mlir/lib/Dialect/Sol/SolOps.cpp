...
Value mlir::sol::emitZeroedVal(OpBuilder &b, Type ty, Location loc) {
  ...
  if (auto contractTy = dyn_cast<ContractType>(ty)) {
    ...
    return b.create<ContractCastOp>(loc, contractTy, zeroAddr);
  }
  ...
  if (isa<ArrayType, StructType, StringType>(ty)) {
    ...
    // TODO (carried from SolidityToMLIR): zero-init memory strings?
    bool zeroInit = !isa<StringType>(ty);
    return b.create<MallocOp>(loc, ty, zeroInit, /*size=*/Value{});
  }

  llvm::errs() << "sol::emitZeroedVal: unsupported type: " << ty << "\n";
  llvm_unreachable("sol::emitZeroedVal: unsupported type");
}
